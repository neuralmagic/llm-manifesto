...
def test_render_omits_user_from_generated_names_by_default(capsys):
    ...
    assert "--user tester" not in rendered
    assert "append --user USER" in rendered
...
def test_render_cli_prefixes_manifest_with_generation_command(monkeypatch, capsys):
    monkeypatch.setenv("MANIFESTO_NAMESPACE", "workload-ns")
    monkeypatch.setattr(workflow, "_source_revision", lambda: "a" * 40)
    rc = main(
        [
            "render",
            "manifest",
    ...
    assert (
        "#   uvx --from "
        f"git+https://github.com/neuralmagic/llm-manifesto@{'a' * 40} "
        "manifesto render manifest \\\n"
    ) in out
    assert f"#     --vllm-env /mnt/shared/tester/vllm-envs/feature\n" in out
    assert str(MODEL) not in out
    assert str(CLUSTER) not in out
    assert "--namespace workload-ns" not in out
    assert "--user tester" not in out
    assert "#     <(cat <<'MANIFESTO_MODEL'\n" in out
    assert "# release: wide-ep-1p-ep8-1d-ep8\n" in out
    assert "#     --cluster <(cat <<'MANIFESTO_CLUSTER'\n" in out
    assert "# name: example-gb200\n" in out
    assert (
        "# To set identity or placement, append --user USER and/or "
        "--namespace NAMESPACE.\n"
    ) in out
    assert (
        f"# Source: https://github.com/neuralmagic/llm-manifesto/tree/{'a' * 40}\n"
        in out
    )
...
def test_render_file_uses_env_defaults(tmp_path, monkeypatch, capsys):
    ...
    assert "#   uvx --from git+https://github.com/" in rendered
    assert "#     --vllm-env /mnt/shared/tester/vllm-envs/feature\n" in rendered
    assert str(MODEL) not in rendered
    assert str(CLUSTER) not in rendered
